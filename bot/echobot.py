...
os.chdir(cwd)
os.listdir(cwd)
data = pd.read_csv('20171110_InventarioFuentes-missing_AYU.csv')
data.columns = ['id_OSM','latitude','longitude','coordinates']
    ...
    for update in updates["result"]:
        ...
        try:
            ...
            if text == 'Comenzamos!' or text == 'Comenzamos de nuevo!':
                keyboard = build_keyboard(['Reto del día','Uno al azar','Salir'])
            ...
            if text == 'Sí, existe':
                keyboard = build_keyboard(['Sí, funciona!','No, no funciona','Salir'])
            ...
            if text == 'Otro' or text =='Otro cercano':
            ...
            try:
                ...
                send_message('¿Aquí hay una fuente?',chat, keyboard)
                send_location(location, chat, date)
...
def get_close_test(location,data, chat_id,random_opt = False):
    ...
    location['latitude'] = data.loc[0]['latitude']
    location['longitude'] = data.loc[0]['longitude']
